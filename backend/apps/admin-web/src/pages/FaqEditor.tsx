  ...
  return (
    <div>
      <PageHelp
        storageKey="faq"
        title="Редактор FAQ"
        summary="Управление разделом «Вопросы и ответы» в мобильном приложении. FAQ структурирован: секция → список вопросов с ответами. На мобайле всё это отдаётся аккордеоном на экране «Помощь»."
        bullets={[
          'Сначала создайте секцию (например «Оплата», «Этапы»). Порядок определяется в БД, секции создаются в конец списка.',
          'Внутри секции добавляйте вопрос + ответ. Ответ — обычный текст, переносы строк сохраняются.',
          'Кнопка «Удалить» рядом с вопросом убирает запись безвозвратно. Удалить всю секцию через UI пока нельзя.',
          'Изменения публикуются сразу — на мобайле подхватятся при следующем открытии экрана FAQ.',
        ]}
      />
      <div className="card">
