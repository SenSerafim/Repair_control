...
class MethodologyRepository {
  ...
  Future<List<MethodologySection>> listSections() => _call(() async {
    final r = await _dio.get<List<dynamic>>('/api/methodology/sections');
    return r.data!
  ...
  Future<List<MethodologySearchHit>> search(String query, {int limit = 20}) =>
      _call(() async {
        final r = await _dio.get<Map<String, dynamic>>(
        ...
        final hits = (r.data?['hits'] as List<dynamic>? ?? const [])
            .map((e) => MethodologySearchHit.parse(e as Map<String, dynamic>))
            .toList();
        return hits;
      });
...
final methodologyRepositoryProvider = Provider<MethodologyRepository>((ref) {
