...
### Качество решений
- Делаем правильно, без костылей и временных обходов. Если баг виден в UI, сначала отделяем симптом от причины: UI можно поправить для читаемости, но источник ошибки в данных/бэкенде/контракте должен быть найден и закрыт отдельным чистым фиксом.
- Не скрываем проблему декоративной правкой, если есть риск повторения. Нужны понятные инварианты, тесты на критичную логику и аккуратная миграция/нормализация данных, когда причина уже попала в прод.
